...
def admin_sell_ticket():
    ...
    for seat_id in seat_ids:
        existing = conn.execute("SELECT id FROM bookings WHERE session_id=? AND seat_id=? AND status != 'cancelled'",
                                (session_id, int(seat_id))).fetchone()
        if existing:
            conn.close()
            return {'error': f'Место {seat_id} уже занято'}, 400
        conn.execute('INSERT INTO bookings (user_id, session_id, seat_id, booked_at, status) VALUES (?,?,?,?,?)',
                     (client['id'], session_id, int(seat_id), datetime.now().isoformat(), 'paid'))
...
def admin_hall_edit(hall_id):
    ...
    old = conn.execute('SELECT capacity, rows, seats_per_row FROM halls WHERE id=?', (hall_id,)).fetchone()
